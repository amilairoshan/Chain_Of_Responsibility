...
public class PeoplesBankHandler extends Handler {

	@Override
	public FixedDeposit calculateFixedDeposit(FixedDeposit fixedDeposit) {
		
		if(fixedDeposit.getBank().equals(Bank.PEOPLES)) {
			...
			if(period <=2) {
				...
				fixedDeposit.setInterest(amount.multiply(BigDecimal.valueOf(.150)));
			}
